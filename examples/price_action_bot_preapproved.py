"""
Turbine Price Action Bot (Pre-Approved USDC Version)

Same price action strategy as price_action_bot.py but uses pre-approved USDC
instead of per-order permit signatures. This eliminates permit nonce management
and enables faster order submission.

Key Differences from permit-based version:
- Uses pre-approved USDC allowances (on-chain approve tx per market)
- Order size and max position configured in USDC terms (not shares)
- Auto-approves USDC when entering a new market
- No permit signing overhead per order
...
Usage:
    TURBINE_PRIVATE_KEY=0x... python examples/price_action_bot_preapproved.py
...
DEFAULT_MAX_POSITION_USDC = 10.0  # $10 max position per market
...
class PriceActionBotPreapproved:
    """Price action trader using pre-approved USDC (no permits).

    Tracks positions in USDC terms and auto-approves USDC for new markets.
    """
    ...
    def get_position_usdc(self, market_id: str) -> float:
        """Get current position in USDC for a market."""
        return self.position_usdc.get(market_id, 0.0)

    def can_trade(self, market_id: str, usdc_amount: float) -> bool:
        """Check if trade would exceed max position."""
        current = self.get_position_usdc(market_id)
    ...
    def ensure_settlement_approved(self, settlement_address: str) -> None:
        ...
        Auto-approves if this is a new settlement contract or insufficient allowance.
        """
        # Calculate required approval (2x max position as buffer)
        required_amount = int(self.max_position_usdc * 2 * 1_000_000)

        # Check if already approved enough
        if settlement_address in self.approved_settlements:
            if self.approved_settlements[settlement_address] >= required_amount:
                return
        ...
        if current_allowance >= required_amount:
            print(f"  Existing USDC allowance sufficient: ${current_allowance / 1_000_000:,.2f}")
            self.approved_settlements[settlement_address] = current_allowance
            return

        # Need to approve
        print(f"\n{'='*50}")
        print(f"APPROVING USDC FOR SETTLEMENT CONTRACT")
        print(f"{'='*50}")
        print(f"Settlement: {settlement_address}")
        print(f"Current allowance: ${current_allowance / 1_000_000:,.2f}")
        print(f"Approving: ${required_amount / 1_000_000:,.2f}")

        try:
            tx_hash = self.client.approve_usdc(required_amount, spender=settlement_address)
            print(f"TX: {tx_hash}")
                ...
                try:
                    ...
                    if receipt:
                        if receipt["status"] == 1:
                            ...
                            self.approved_settlements[settlement_address] = required_amount
                        else:
        ...
        try:
            ...
            if resolved_txs:
                ...
                for trade in my_recent_trades:
                    ...
                    self.position_usdc[self.market_id] = self.get_position_usdc(self.market_id) + usdc_spent

                    outcome_str = "YES" if trade.outcome == 0 else "NO"
                    print(f"  Filled: ${usdc_spent:.2f} USDC → {trade.size / 1_000_000:.2f} {outcome_str} shares")
        ...
        try:
            ...
            for position in positions:
                if position.market_id == self.market_id:
                    ...
                    if total_shares > 0:
                        print(f"Position synced: ~${estimated_usdc:.2f} USDC in shares")
                        self.position_usdc[self.market_id] = estimated_usdc
                    else:
                        print("Position synced: No existing positions")
                        self.position_usdc[self.market_id] = 0.0
                    return
    ...
    async def execute_signal(self, action: str, confidence: float) -> None:
        """Execute the trading signal using pre-approved USDC (no permits)."""
        ...
        if not self.can_trade(self.market_id, self.order_size_usdc):
            current = self.get_position_usdc(self.market_id)
            print(f"Position limit reached: ${current:.2f} / ${self.max_position_usdc:.2f}")
            return
        ...
        try:
            # Create order WITHOUT permit (using pre-approved allowance)
            ...
            # NO permit signature - relying on pre-approved allowance
            ...
            if result and isinstance(result, dict):
                ...
                print(f"→ Order submitted: {outcome_str} @ {price / 10000:.1f}% | ${self.order_size_usdc:.2f} = {shares/1_000_000:.4f} shares (status: {status})")
                ...
                try:
                    ...
                    if my_pending:
                        ...
                        print(f"⏳ Order PENDING on-chain (TX: {pending.tx_hash[:16]}...)")
                ...
                try:
                    ...
                    if my_trades:
                        ...
                        print(f"✓ FILLED: ${usdc_spent:.2f} USDC → {trade.size / 1_000_000:.4f} shares")
                        return
                except Exception as e:
                ...
                try:
                    ...
                    if matching:
                        ...
                        print(f"⚠ Order not found - may have been rejected")
                except Exception as e:
            ...
            try:
                ...
                if action != "HOLD":
                    ...
                    if current_price > 0:
                        ...
                        pos = self.get_position_usdc(self.market_id)
                        print(f"BTC ${current_price:,.2f} ({diff_pct:+.2f}% from ${strike_usd:,.2f}) | Pos: ${pos:.2f}/${self.max_position_usdc:.2f} - HOLD")
    ...
    async def switch_to_new_market(self, new_market_id: str, start_price: int = 0) -> None:
        """Switch to a new market and auto-approve USDC."""
        ...
        # Auto-approve USDC for this settlement contract
        ...
        print(f"Trading market: {new_market_id[:8]}... | Strike: ${strike_usd:,.2f}")
            ...
            try:
                ...
                if time_remaining <= 60 and time_remaining > 0:
                    if not self.market_expiring:
                        print(f"⏰ Market expires in {time_remaining}s - stopping trades")
...
async def main():
    parser = argparse.ArgumentParser(
        description="Price action bot with pre-approved USDC (no permits)"
    )
    parser.add_argument(
        "-s", "--order-size",
        type=float,
    ...
    parser.add_argument(
        ...
        help=f"Max position per market in USDC (default: ${DEFAULT_MAX_POSITION_USDC})"
    )
